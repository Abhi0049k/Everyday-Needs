...
orderRouter.get('/bookedOrders', async(req, res)=>{
    try{
        const {cusId} = req.body;
...
orderRouter.patch('/checkout', async(req, res)=>{
    try{
        let {cusId} = req.body;
...
orderRouter.patch('/increQty/:id', async(req, res)=>{
    try{
        const {cusId} = req.body;
        const {id} = req.params;
...
orderRouter.patch('/decreQty/:id', async(req, res)=>{
    try{
        const {cusId} = req.body;
        const {id} = req.params;
        const order = await orderModel.findOne({userId: cusId, _id: id});
